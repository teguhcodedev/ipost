          ...
          <div class="modal-body">
              ...
              <div class="row">
                <div class="col-12 text-center">
                  <img src="{{ asset('chubb_foto/default.jpg') }}" class="img-edit">
                </div>
              ...
              <div class="form-group row">
                <label class="col-3 col-form-label font-weight-bold">Blokir</label>
                <div class="col-9">
                  <select class="form-control" name="is_banned">
                    <option value="0">Buka Blokir</option>
              ...
              <div class="form-group row">
                ...
                <div class="col-9">
                  <select class="form-control" name="role">
                    ...
                    <option value="asisten manager">AM</option>
  ...
  <div class="col-md-12 grid-margin">
    <div class="card card-noborder b-radius">
      <div class="card-body">
        	...
        	<div class="col-12 table-responsive">
              ...
              <thead>
                <tr>
                  ...
                  <th>Last Update</th>
                  <th>Join Date</th>
                  <th>Resign/Terminated</th>
                  <th>Status Login</th>
                  <th>Status Lock</th>
                  <th>Can Use Softphone</th>
                  <th>Create User</th>
                  <th>Edit User</th>
                  <th></th>
                </tr>
              ...
              <tbody>
                ...
                <tr>
                  <td>
                  	<img src="{{ asset('chubb_foto/' . $user->AGENT_PHOTO) }}">
                  	<span class="ml-1">{{ $user->AGENT_USERNAME }}</span>
                  </td>
                  <td>{{ $user->AGENT_NAME }}</td>
                  <td>{{ $user->AGENT_LEVEL }}</td>
                  <td>{{ $user->GROUP_ID }}</td>
                  <td>{{ $user->SPV_ID }}</td>
                  <td>{{ $user->MANAGER_ID }}</td>
                 
                  <td>
                    @if($user->STS_AUDITORIAL=="preview")
                    <span class="btn btn-success">PREVIEW</span>
                    @else
                    <span class="btn btn-warning">PROGRESSIVE</span>
                    @endif
                  </td>
                  <td>{{ $user->AGENT_DN}}</td>
                  <td>
                    ...
                    @else
                    <span class="btn btn-danger">RESIGN</span>
                    @endif
                  </td>
                  ...
                  <td>
                    {{Carbon\Carbon::parse($user->DATE_JOIN)->format('d F Y')}}
                  </td>
                  <td>{{Carbon\Carbon::parse($user->DATE_RESIGN )->format('d F Y')}}</td>
                  <td>
                    @if ($user->LOGIN_STATUS==1)
                    <span class="text-success" style="font-weight: bold">LOGIN</span>
                    @else
                      <span class="text-danger" style="font-weight: bold">Offline</span>
                    @endif
                  </td>
                  ...
                  <td>
                    ...
                    @else
                    <span class="btn btn-primary">Not Locked</span>
                    @endif
                  </td>
                  <td class="text-center">
                    @if ($user->CAN_USE_SOFTPHONE==1)
                  ...
                  <td>
                    ...
                    <a href="{{url('chubblife/edit/'.$user->id)}}" class="btn btn-edit btn-icons btn-rounded btn-secondary">
                      <i class="mdi mdi-pencil"></i>
                   </a>
                    <button type="button" class="btn btn-edit btn-icons btn-rounded btn-secondary" data-toggle="modal" data-target="#editModal" data-edit="{{ $user->id }}">
        ...
        <div class="mt-3 float-right">{!! $MUA->links() !!}</div>
      </div>
    </div>
  </div>
...
<script type="text/javascript">
  ...
  $(document).on('click', '.btn-edit', function(){
    ...
    $.ajax({
      ...
      {
        $('.img-edit').attr("src", "{{ asset('chubb_foto') }}/" + response.user.foto);
  ...
  $(document).on('click', '.btn-delete-img', function(){
    $(".img-edit").attr("src", "{{ asset('chubb_foto') }}/default.jpg");
