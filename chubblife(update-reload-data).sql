...
--
-- Host: 127.0.0.1
-- Generation Time: May 24, 2021 at 06:03 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spComboboxReload` (IN `in_criteria` VARCHAR(255), IN `in_user` VARCHAR(225), IN `in_campaign_name` VARCHAR(225), IN `in_level_selected` VARCHAR(225))  BEGIN
DECLARE vLevel varchar(255);
DECLARE vWhere  varchar(500);
DECLARE vQuery varchar(1000);
...
IF in_criteria = 'CAMPAIGN_RELOAD'
		THEN
	  (SELECT AGENT_LEVEL FROM master_user_agents WHERE AGENT_USERNAME = in_user LIMIT 1) INTO vLevel;
     ...
     IF ((vLevel = 'ADMINISTRATOR') OR (vLevel = 'ADMIN'))
    		THEN
			SET vWhere = " AND IFNULL(DATA_MGR,'') NOT IN ('')  ";
			
		ELSEIF vLevel = 'MANAGER' 	
			THEN
			SET vWhere = CONCAT(" AND IFNULL(DATA_MGR,'') = '",in_user,"' AND IFNULL(DATA_SPV,'') NOT IN ('') ");
		ELSEIF vLevel = 'SPV'
        	THEN
			SET vWhere = CONCAT(" AND IFNULL(DATA_SPV,'') = '",in_user,"' AND IFNULL(AGENT_USERNAME,'') NOT IN ('') ");
		END  IF; /*'tutup vlevel*/
        	
					SET vQuery =  CONCAT("SELECT DISTINCT (CAMPAIGN_NAME) AS CBO FROM MASTER_CUSTOMER ",
													 " WHERE DATE_FORMAT(EXPIRE_DATE, '%Y-%m-%d')>=DATE_FORMAT(NOW(), '%Y-%m-%d') ",
													 " AND (IFNULL(RESULT,'') NOT IN ('Interested','Block'))",
														 vWhere," ORDER BY CAMPAIGN_NAME DESC ;");  
      
    PREPARE stmt1 FROM vQuery;
	  EXECUTE stmt1; 
	  DEALLOCATE PREPARE stmt1; 

END IF; /*tutup if  in_criteria = 'CAMPAIGN_RELOAD'*/

IF in_criteria = 'CAMPAIGN_RELOAD_STATUSDATA_SUMMARY'
		THEN
	  (SELECT AGENT_LEVEL FROM master_user_agents WHERE AGENT_USERNAME = in_user LIMIT 1) INTO vLevel;
     ...
     IF ((vLevel = 'ADMINISTRATOR') OR (vLevel = 'ADMIN'))
		...
		END  IF; /*'tutup vlevel*/
				...
				SET vQuery = CONCAT("SELECT COUNT(*) AS TOTAL_DATA FROM MASTER_CUSTOMER ", 
																...
																" AND IFNULL(RESULT,'') NOT IN ('Interested','Block') "
																" AND CAMPAIGN_NAME = '",in_campaign_name,"' ",
																 vWhere,
																	" ;"
                             );
...
END IF; /*tutup if  in_criteria = 'CAMPAIGN_RELOAD_STATUSDATA_SUMMARY'*/


IF in_criteria = 'CAMPAIGN_RELOAD_STATUSDATA'
		THEN
	  (SELECT AGENT_LEVEL FROM master_user_agents WHERE AGENT_USERNAME = in_user LIMIT 1) INTO vLevel;
     ...
     IF ((vLevel = 'ADMINISTRATOR') OR (vLevel = 'ADMIN'))
        ...
        SET vQuery =  CONCAT("SELECT DISTINCT ",
														 " CASE ",
														 " WHEN IFNULL(DIAL_STATUS,'')  = '' THEN 'Fresh Data' ",
														 " ELSE IFNULL(DIAL_STATUS,'') END AS STATUS, ",
														 " IFNULL(RESULT_DETAIL,'') AS DETAIL_STATUS ",
...
END IF; /*tutup if  in_criteria = 'CAMPAIGN_RELOAD_STATUSDATA'*/
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetReload_List` (IN `in_criteria` VARCHAR(255), IN `in_user` VARCHAR(225), IN `in_selected_level` VARCHAR(225), IN `in_campaign_name` VARCHAR(225), IN `in_where_status` VARCHAR(500), IN `in_where_detail_status` VARCHAR(500))  BEGIN
DECLARE vLevel varchar(255);
DECLARE vWhere  varchar(500);
DECLARE vWhere2 varchar(500);
DECLARE vWhere3 varchar(500);
DECLARE where_status VARCHAR(500);
DECLARE where_detail_status VARCHAR(500);
DECLARE vQuery varchar(1000);
   (SELECT AGENT_LEVEL FROM master_user_agents WHERE AGENT_USERNAME = in_user LIMIT 1) INTO vLevel;
   
	 SET where_status = CONCAT(" AND IFNULL(DIAL_STATUS,'') IN ('",in_where_status,"') ");
	 SET where_detail_status = CONCAT(" AND IFNULL(RESULT_DETAIL,'') IN ('",in_where_detail_status,"') ");
	 
   IF in_criteria = 'RELOAD'
   THEN
   
  		IF vLevel = 'ADMINISTRATOR' OR  @vLevel = 'ADMIN'
        	THEN
				set vWhere2 = ' ' ;
				set vWhere3 = " AND IFNULL(DATA_MGR,'') NOT IN ('')  ";
    	...
    	ELSEIF vLevel = 'MANAGER'
			THEN
				set vWhere2 = CONCAT(" AND IFNULL(MANAGER_ID,'') = '",in_user, "' ");
				set vWhere3 = CONCAT(" AND IFNULL(DATA_MGR,'') = '",in_user, "' AND IFNULL(DATA_SPV,'') NOT IN ('')  ");
    		...
    		THEN
				set vWhere2 = CONCAT(" AND IFNULL(SPV_ID,'') = '",in_user,"' ");
				set vWhere3 = CONCAT(" AND IFNULL(DATA_SPV,'') = '",in_user, "' AND IFNULL(AGENT_USERNAME,'') NOT IN ('')  ");
				IF in_selected_level = 'AGENT'
					THEN
					set vWhere = " IFNULL(AGENT_USERNAME,'') = master_user_agents.AGENT_USERNAME ";
     ...
     SET vQuery =  CONCAT(" SELECT AGENT_USERNAME ",
                          " ,AGENT_NAME ",
                           ...
                           " AND  DATE_FORMAT(EXPIRE_DATE, '%Y-%m-%d')>=DATE_FORMAT(NOW(),'%Y-%m-%d') ",
													 ...
													 where_detail_status,
													 ") AS LEAD_DATA ", 
													 " FROM master_user_agents ",
													" WHERE AGENT_LEVEL = '",in_selected_level,
                          "' AND AGENT_STATUS = 'ACTIVE' ",
                           vWhere2," ORDER BY AGENT_USERNAME ;"
					  ); 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spSaveDist_Data` (IN `in_user` VARCHAR(255), IN `in_filter_group` VARCHAR(255), IN `in_lower_age` VARCHAR(225), IN `in_upper_age` VARCHAR(225), IN `in_type_card` VARCHAR(225), IN `in_selected_level` VARCHAR(225), IN `in_statusdata1` VARCHAR(225), IN `in_statusdata2` VARCHAR(225), IN `in_total_dist` VARCHAR(225), IN `in_user_dist` VARCHAR(225), IN `in_campaign_name` VARCHAR(500), IN `in_zipcode` VARCHAR(225), IN `in_gender` VARCHAR(225), IN `in_event_operation` VARCHAR(225))  BEGIN
	...
	DECLARE event_desc VARCHAR(100);
	DECLARE vLevel varchar(255);
			...
			/*--END OF ADD FILTER FUNCTION */
		 	...
		 	IF vLevel = 'ADMINISTRATOR'
		       ...
		       SET @vQuery = CONCAT(" INSERT INTO DATA_ASSIGNMENT (ASSIGN_DATE,ID_MASTER,CUST_APPID ", 
					...
					SET event_desc = CONCAT("DISTRIBUTION CAMPAIGN_NAME :  ",in_campaign_name,
																					" TO AGENT ",in_user_dist,
																					" TOTAL DATA : ",in_total_dist);
	   
					INSERT INTO EVENT_AUDIT_TRAIL (EVENT_TIME
																				,AGENT_USERNAME
																				,EVENT_TABLE
																				,EVENT_STATUS
																				,EVENT_OPERATION
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spSaveReload_Data` (IN `in_user` VARCHAR(255), IN `in_campaign_name` VARCHAR(500), IN `in_selected_level` VARCHAR(225), IN `in_statusdata1` VARCHAR(225), IN `in_statusdata2` VARCHAR(225), IN `in_user_reload` VARCHAR(225), IN `in_total_reload` VARCHAR(225), IN `in_event_operation` VARCHAR(225))  BEGIN
	DECLARE StatusAssign varchar(500);
	DECLARE Flag_Status varchar(500);
	DECLARE vLevel varchar(255);
	DECLARE where_status VARCHAR(500);
   ...
   (SELECT AGENT_LEVEL FROM master_user_agents WHERE AGENT_USERNAME = in_user LIMIT 1) INTO vLevel;
	 	...
	 	SET where_status = CONCAT(" AND IFNULL(DIAL_STATUS,'') IN ('",in_statusdata1,"') ");
		SET where_detail_status = CONCAT(" AND IFNULL(RESULT_DETAIL,'') IN ('",in_statusdata2,"') ");
	 	IF vLevel = 'ADMINISTRATOR'
				THEN
				If in_selected_level = 'AGENT'
							THEN
						 IF in_statusdata1 = '' and in_statusdata2 = ''
								THEN
									SET StatusAssign = "RECALL_ADM_FROM_AGENT_NEW";
									SET Flag_Status="2" ;
							ELSE
									SET StatusAssign = "RECALL_ADM_FROM_AGENT_RE";
									SET Flag_Status= "4" ;
								END IF; 	/* end if in_statusdata1 = '' and in_statusdata2 = ''*/
		       ...
		       SET @vQuery = CONCAT(" INSERT INTO DATA_ASSIGNMENT (ASSIGN_DATE,ID_MASTER,CUST_APPID ", 
															  ",AGENT_USERNAME,AGENT_USERNAME_TO,ASSIGN_STATUS,CREATE_BY) ",
																" SELECT  NOW() AS ASSIGN_DATE,ID,CUST_APPID, '",in_user_reload,
																"' AS AGENT_USERNAME, '' AS AGENT_USERNAME_TO, '",StatusAssign
																,"' AS ASSIGN_STATUS, '",in_user,
																"' as alter_BY FROM MASTER_CUSTOMER WHERE IFNULL(FLAG_OVERRIDE,'') <> '2' ",
																" AND IFNULL(USER_LOCK,'') = '' ",
																" AND DATE_FORMAT(EXPIRE_DATE, '%Y-%m-%d')>=DATE_FORMAT(NOW(),'%Y-%m-%d') ",
																" AND IFNULL(RESULT,'') NOT IN ('Interested','Block') AND CAMPAIGN_NAME = '",
																in_campaign_name,"' AND IFNULL(AGENT_USERNAME,'') = '",in_user_reload
																,"' AND IFNULL(DATA_MGR,'') NOT IN ('') ",where_status,where_detail_status,
																" ;");
					PREPARE stmt FROM @vQuery;
					EXECUTE stmt; 
					DEALLOCATE PREPARE stmt; 
					...
					SET @vQuery1 = CONCAT(" UPDATE MASTER_CUSTOMER SET AGENT_USERNAME = '',", 
																" FLAG_STATUS = CASE ",
																"	WHEN IFNULL(DIAL_STATUS,'') = '' THEN '2'",
																" ELSE '4' END ", 
																" WHERE IFNULL(FLAG_OVERRIDE,'') <> '2' ",
																" AND IFNULL(USER_LOCK,'') = '' ",
																" AND DATE_FORMAT(EXPIRE_DATE, '%Y-%m-%d')>=DATE_FORMAT(NOW(),'%Y-%m-%d') ",
																" AND IFNULL(RESULT,'') NOT IN ('Interested','Block') AND CAMPAIGN_NAME = '",
																in_campaign_name,"' AND IFNULL(AGENT_USERNAME,'') = '",in_user_reload
					...
					SET event_desc = CONCAT("RELOAD CAMPAIGN_NAME :  ",in_campaign_name,
																					" FROM ",in_user_reload," TO ADMINISTRATOR ",in_user,
																					" TOTAL DATA : ",in_total_reload);
...
-- 				
			ELSEIF in_selected_level = 'SPV'
					THEN
						 IF (in_statusdata1 = '' and in_statusdata2 = '')
								THEN
									SET StatusAssign = "RECALL_ADM_FROM_SPV_NEW";
									SET Flag_Status= "2";
							ELSE
									SET StatusAssign = "RECALL_ADM_FROM_SPV_RE";
									SET Flag_Status= "4";
								END IF; 	/* end if in_statusdata1 = '' and in_statusdata2 = ''*/
		       ...
		       SET @vQuery = CONCAT(" INSERT INTO DATA_ASSIGNMENT (ASSIGN_DATE,ID_MASTER,CUST_APPID ", 
															  ",AGENT_USERNAME,AGENT_USERNAME_TO,ASSIGN_STATUS,CREATE_BY) ",
																...
																" AND DATE_FORMAT(EXPIRE_DATE, '%Y-%m-%d')>=DATE_FORMAT(NOW(),'%Y-%m-%d') ",
																" AND IFNULL(RESULT,'') NOT IN ('Interested','Block') AND CAMPAIGN_NAME = '",
																in_campaign_name,"' AND IFNULL(DATA_SPV,'') = '",in_user_reload
																,"' AND IFNULL(DATA_MGR,'') NOT IN ('') ",where_status,where_detail_status,
																" ;");
					PREPARE stmt FROM @vQuery;
					EXECUTE stmt; 
					...
					SET @vQuery1 = CONCAT(" UPDATE MASTER_CUSTOMER SET AGENT_USERNAME = '', DATA_SPV = '' ,",
																" FLAG_STATUS = CASE ",
																							"	WHEN IFNULL(DIAL_STATUS,'') = '' THEN '2'",
																							" ELSE '4' END ", 
																" WHERE IFNULL(FLAG_OVERRIDE,'') <> '2' ",
			...
			ELSEIF in_selected_level = 'MANAGER'
					THEN
						 ...
						 IF (in_statusdata1 = '' and in_statusdata2 = '')
								THEN
									SET StatusAssign = "RECALL_ADM_FROM_MGR_NEW";
									SET Flag_Status= "2";
							ELSE
									SET StatusAssign = "RECALL_ADM_FROM_MGR_RE";
									SET Flag_Status= "4";
								END IF; 	/* end if in_statusdata1 = '' and in_statusdata2 = ''*/
		       ...
		       SET @vQuery = CONCAT(" INSERT INTO DATA_ASSIGNMENT (ASSIGN_DATE,ID_MASTER,CUST_APPID ", 
															  ",AGENT_USERNAME,AGENT_USERNAME_TO,ASSIGN_STATUS,CREATE_BY) ",
																...
																" AND DATE_FORMAT(EXPIRE_DATE, '%Y-%m-%d')>=DATE_FORMAT(NOW(),'%Y-%m-%d') ",
																" AND IFNULL(RESULT,'') NOT IN ('Interested','Block') AND CAMPAIGN_NAME = '",
																in_campaign_name,"' AND IFNULL(DATA_MGR,'') = '",in_user_reload
																,"' AND IFNULL(DATA_MGR,'') NOT IN ('') ",where_status,where_detail_status,
																" ;");
					PREPARE stmt FROM @vQuery;
					EXECUTE stmt; 
					...
					SET @vQuery1 = CONCAT(" UPDATE MASTER_CUSTOMER SET AGENT_USERNAME = '', DATA_SPV = '' ,DATA_MGR = '',",
																" FLAG_STATUS = CASE ",
																						"	WHEN IFNULL(DIAL_STATUS,'') = '' THEN '2'",
																						" ELSE '4' END ", 
																" WHERE IFNULL(FLAG_OVERRIDE,'') <> '2' ",
					...
					INSERT INTO EVENT_AUDIT_TRAIL (EVENT_TIME
																...
																Values (NOW()
																				...
																				,''
																				,'');										
		END IF ; 	/* end if in_selected_level agent dari vlevel mgr */
	 	...
	 	ELSEIF vLevel = 'MANAGER'
				...
				If in_selected_level = 'AGENT'
						 ...
						 IF (in_statusdata1 = '' and in_statusdata2 = '')
								THEN
									SET StatusAssign = "RECALL_MGR_FROM_AGENT_NEW";
									SET Flag_Status= "2";
							ELSE
									SET StatusAssign = "RECALL_MGR_FROM_AGENT_RE";
									SET Flag_Status= "4";
								END IF; 	/* end if in_statusdata1 = '' and in_statusdata2 = ''*/
					...
					SET @vQuery = CONCAT(" INSERT INTO DATA_ASSIGNMENT (ASSIGN_DATE,ID_MASTER,CUST_APPID ", 
															  ",AGENT_USERNAME,AGENT_USERNAME_TO,ASSIGN_STATUS,CREATE_BY) ",
																...
																" AND DATE_FORMAT(EXPIRE_DATE, '%Y-%m-%d')>=DATE_FORMAT(NOW(),'%Y-%m-%d') ",
																" AND IFNULL(RESULT,'') NOT IN ('Interested','Block') AND CAMPAIGN_NAME = '",
																in_campaign_name,"' AND AGENT_USERNAME = '",in_user_reload,"' AND IFNULL(DATA_MGR,'') = '",																	  
																in_user,"' AND IFNULL(DATA_SPV,'') NOT IN ('') ",where_status,where_detail_status,
																" ;");
					PREPARE stmt FROM @vQuery;
					EXECUTE stmt; 
					DEALLOCATE PREPARE stmt;
					...
					SET @vQuery1 = CONCAT(" UPDATE MASTER_CUSTOMER SET AGENT_USERNAME = '',",
																...
																" AND DATE_FORMAT(EXPIRE_DATE, '%Y-%m-%d')>=DATE_FORMAT(NOW(),'%Y-%m-%d') ",
																" AND IFNULL(RESULT,'') NOT IN ('Interested','Block') AND CAMPAIGN_NAME = '",
																in_campaign_name,"' AND AGENT_USERNAME = '",in_user_reload,
																"' AND DATA_MGR = '",in_user
																,"' AND IFNULL(DATA_SPV,'') NOT IN ('') ",where_status,where_detail_status,
																" ;");
					...
					SET event_desc = CONCAT("RELOAD CAMPAIGN_NAME :  ",in_campaign_name,
																					" FROM ",in_user_reload," TO MANAGER ",in_user,
																					" TOTAL DATA : ",in_total_reload);
			...
			ELSEIF in_selected_level = 'SPV'
					THEN
						 IF (in_statusdata1 = '' and in_statusdata2 = '')
								THEN
									SET StatusAssign = "RECALL_MGR_FROM_SPV_NEW";
									SET Flag_Status= "2";
							ELSE
									SET StatusAssign = "RECALL_MGR_FROM_SPV_RE";
									SET Flag_Status= "4";
								END IF; 	/* end if in_statusdata1 = '' and in_statusdata2 = ''*/
					...
					SET @vQuery = CONCAT(" INSERT INTO DATA_ASSIGNMENT (ASSIGN_DATE,ID_MASTER,CUST_APPID ", 
															  ",AGENT_USERNAME,AGENT_USERNAME_TO,ASSIGN_STATUS,CREATE_BY) ",
																...
																" AND IFNULL(RESULT,'') NOT IN ('Interested','Block') AND CAMPAIGN_NAME = '",
																in_campaign_name,"' AND IFNULL(DATA_SPV,'')  = '",in_user_reload,
																"' AND IFNULL(DATA_MGR,'') = '",in_user,"' ",where_status,where_detail_status,
																" ;");
					PREPARE stmt FROM @vQuery;
					EXECUTE stmt; 
					DEALLOCATE PREPARE stmt;
		...
		END IF ; 	/* end if in_selected_level dari vlevel MANAGER */		
		 	ELSEIF vLevel = 'SPV'
				...
				If in_selected_level = 'AGENT'
						 ...
						 IF in_statusdata1 = '' and in_statusdata2 = ''
								THEN
									SET StatusAssign = "RECALL_SPV_FROM_AGENT_NEW";
									SET Flag_Status="2" ;
							ELSE
									SET StatusAssign = "RECALL_SPV_FROM_AGENT_RE";
									SET Flag_Status= "4" ;
								END IF; 	/* end if in_statusdata1 = '' and in_statusdata2 = ''*/
					...
					SET @vQuery = CONCAT(" INSERT INTO DATA_ASSIGNMENT (ASSIGN_DATE,ID_MASTER,CUST_APPID ", 
															  ",AGENT_USERNAME,AGENT_USERNAME_TO,ASSIGN_STATUS,CREATE_BY) ",
																...
																" AND IFNULL(RESULT,'') NOT IN ('Interested','Block') AND CAMPAIGN_NAME = '",
																in_campaign_name,"' AND IFNULL(AGENT_USERNAME,'')  = '",in_user_reload,
																"' AND IFNULL(DATA_SPV,'') = '",in_user,"' ",where_status,where_detail_status,
																" ;");
					PREPARE stmt FROM @vQuery;
					EXECUTE stmt; 
					DEALLOCATE PREPARE stmt;
				  ...
				  SET @vQuery1 = CONCAT(" UPDATE MASTER_CUSTOMER SET AGENT_USERNAME = '' ",
					...
					SET event_desc = CONCAT("RELOAD CAMPAIGN_NAME :  ",in_campaign_name,
																					" FROM ",in_user_reload," TO SPV ",in_user,
																					" TOTAL DATA : ",in_total_reload);
		...
		END IF ; 	/* end if in_selected_level spv dari vlevel SPV */		
		END IF ;  /* end if akhirr vLevel =*/ 
...
(169, '2021-05-23 19:22:17', 0, 52, 'Tofan', '', 'NEW_ASSIGNMENT_MGR_TO_SPV', 'Intan'),
(173, '2021-05-24 09:08:13', 0, 42, 'Gerung', '', 'NEW_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(174, '2021-05-24 09:23:15', 0, 42, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(175, '2021-05-24 09:24:14', 0, 42, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'tofan'),
(176, '2021-05-24 09:25:59', 0, 42, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(180, '2021-05-24 09:58:26', 0, 42, 'Tofan', '', 'NEW_RE_ASSIGNMENT_MGR_TO_SPV', 'Intan'),
(181, '2021-05-24 09:59:00', 0, 43, 'Gerung', '', 'NEW_RE_ASSIGNMENT_MGR_TO_AGENT', 'Intan'),
(182, '2021-05-24 10:02:15', 0, 42, 'tofan', '', 'RECALL_MGR_FROM_SPV_NEW', 'Intan'),
(183, '2021-05-24 10:02:15', 0, 43, 'tofan', '', 'RECALL_MGR_FROM_SPV_NEW', 'Intan'),
(185, '2021-05-24 10:41:06', 0, 42, 'Gerung', '', 'NEW_RE_ASSIGNMENT_MGR_TO_AGENT', 'Intan'),
(186, '2021-05-24 10:42:25', 0, 43, 'Gerung', '', 'NEW_RE_ASSIGNMENT_MGR_TO_AGENT', 'Intan'),
(187, '2021-05-24 10:48:16', 0, 42, 'tofan', '', 'RECALL_MGR_FROM_SPV_NEW', 'Intan'),
(188, '2021-05-24 10:48:16', 0, 43, 'tofan', '', 'RECALL_MGR_FROM_SPV_NEW', 'Intan'),
(190, '2021-05-24 10:50:29', 0, 42, 'Gerung', '', 'NEW_RE_ASSIGNMENT_MGR_TO_AGENT', 'Intan'),
(191, '2021-05-24 10:50:29', 0, 43, 'Gerung', '', 'NEW_RE_ASSIGNMENT_MGR_TO_AGENT', 'Intan'),
(196, '2021-05-24 10:55:16', 0, 42, 'gerung', '', 'RECALL_MGR_FROM_AGENT_NEW', 'Intan'),
(197, '2021-05-24 10:55:16', 0, 43, 'gerung', '', 'RECALL_MGR_FROM_AGENT_NEW', 'Intan'),
(199, '2021-05-24 11:19:00', 0, 42, 'Intan', '', 'RECALL_ADM_FROM_MGR_NEW', 'busi'),
(200, '2021-05-24 11:19:00', 0, 43, 'Intan', '', 'RECALL_ADM_FROM_MGR_NEW', 'busi'),
(201, '2021-05-24 11:19:00', 0, 44, 'Intan', '', 'RECALL_ADM_FROM_MGR_NEW', 'busi'),
(202, '2021-05-24 12:38:56', 0, 42, 'Intan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_MGR', 'Busi'),
(203, '2021-05-24 12:38:56', 0, 43, 'Intan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_MGR', 'Busi'),
(204, '2021-05-24 12:38:56', 0, 44, 'Intan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_MGR', 'Busi'),
(205, '2021-05-24 12:43:14', 0, 42, 'Tofan', '', 'NEW_ASSIGNMENT_MGR_TO_SPV', 'Intan'),
(206, '2021-05-24 12:43:14', 0, 43, 'Tofan', '', 'NEW_ASSIGNMENT_MGR_TO_SPV', 'Intan'),
(208, '2021-05-24 12:52:19', 0, 42, 'tofan', '', 'RECALL_ADM_FROM_SPV_NEW', 'busi'),
(209, '2021-05-24 12:52:19', 0, 43, 'tofan', '', 'RECALL_ADM_FROM_SPV_NEW', 'busi'),
(211, '2021-05-24 12:53:00', 0, 42, 'Gerung', '', 'NEW_ASSIGNMENT_MGR_TO_AGENT', 'Intan'),
(212, '2021-05-24 12:53:00', 0, 43, 'Gerung', '', 'NEW_ASSIGNMENT_MGR_TO_AGENT', 'Intan'),
(214, '2021-05-24 13:03:09', 0, 42, 'gerung', '', 'RECALL_ADM_FROM_AGENT_NEW', 'busi'),
(215, '2021-05-24 13:03:09', 0, 43, 'gerung', '', 'RECALL_ADM_FROM_AGENT_NEW', 'busi'),
(217, '2021-05-24 13:07:17', 0, 42, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(218, '2021-05-24 13:47:47', 0, 42, 'Intan', '', 'RECALL_ADM_FROM_MGR_NEW', 'busi'),
(219, '2021-05-24 13:47:47', 0, 43, 'Intan', '', 'RECALL_ADM_FROM_MGR_NEW', 'busi'),
(220, '2021-05-24 13:47:47', 0, 44, 'Intan', '', 'RECALL_ADM_FROM_MGR_NEW', 'busi'),
(221, '2021-05-24 13:50:55', 0, 42, 'Intan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_MGR', 'Busi'),
(222, '2021-05-24 13:51:40', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(223, '2021-05-24 13:51:40', 0, 44, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(225, '2021-05-24 13:51:48', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(226, '2021-05-24 13:51:49', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(227, '2021-05-24 13:51:50', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(228, '2021-05-24 13:51:50', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(229, '2021-05-24 13:51:51', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(230, '2021-05-24 13:51:51', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(231, '2021-05-24 13:52:18', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(232, '2021-05-24 13:52:19', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(233, '2021-05-24 13:52:20', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(234, '2021-05-24 13:52:20', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(235, '2021-05-24 13:52:21', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(236, '2021-05-24 13:52:21', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(237, '2021-05-24 13:52:22', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(238, '2021-05-24 13:52:22', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(239, '2021-05-24 13:52:23', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(240, '2021-05-24 13:52:23', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(241, '2021-05-24 13:52:23', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(242, '2021-05-24 13:52:24', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(243, '2021-05-24 13:52:24', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(244, '2021-05-24 13:52:25', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(245, '2021-05-24 13:52:30', 0, 43, 'Intan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_MGR', 'Busi'),
(246, '2021-05-24 13:53:26', 0, 44, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(247, '2021-05-24 13:58:07', 0, 44, 'tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'busi'),
(248, '2021-05-24 13:58:30', 0, 42, 'Intan', '', 'RECALL_ADM_FROM_MGR_NEW', 'busi'),
(249, '2021-05-24 13:58:30', 0, 43, 'Intan', '', 'RECALL_ADM_FROM_MGR_NEW', 'busi'),
(251, '2021-05-24 14:02:27', 0, 42, 'Intan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_MGR', 'Busi'),
(252, '2021-05-24 14:02:52', 0, 43, 'Tofan', '', 'NEW_RE_ASSIGNMENT_ADM_TO_SPV', 'Busi'),
(253, '2021-05-24 14:04:40', 0, 44, 'Gerung', '', 'NEW_RE_ASSIGNMENT_ADM_TO_AGENT', 'Busi'),
(254, '2021-05-24 14:06:36', 0, 42, 'Tofan', '', 'NEW_ASSIGNMENT_MGR_TO_SPV', 'Intan'),
(255, '2021-05-24 14:07:12', 0, 45, 'Gerung', '', 'NEW_ASSIGNMENT_MGR_TO_AGENT', 'Intan'),
(256, '2021-05-24 14:08:14', 0, 42, 'Gerung', '', 'NEW_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(257, '2021-05-24 14:08:14', 0, 43, 'Gerung', '', 'NEW_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(259, '2021-05-24 14:16:22', 0, 52, 'Gerung', '', 'NEW_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(260, '2021-05-24 14:19:31', 0, 47, 'Tofan', '', 'NEW_ASSIGNMENT_MGR_TO_SPV', 'Intan'),
(261, '2021-05-24 21:33:43', 0, 47, 'Tofan', '', 'RECALL_MGR_FROM_SPV_NEW', 'Intan'),
(262, '2021-05-24 21:53:20', 0, 52, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(263, '2021-05-24 21:53:56', 0, 45, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(264, '2021-05-24 22:04:39', 0, 45, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(265, '2021-05-24 22:04:53', 0, 45, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(266, '2021-05-24 22:06:36', 0, 45, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(267, '2021-05-24 22:07:30', 0, 52, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(268, '2021-05-24 22:15:10', 0, 42, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(269, '2021-05-24 22:15:10', 0, 43, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(270, '2021-05-24 22:15:10', 0, 44, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(271, '2021-05-24 22:15:49', 0, 45, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(272, '2021-05-24 22:18:52', 0, 52, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(273, '2021-05-24 22:21:47', 0, 42, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(274, '2021-05-24 22:21:47', 0, 43, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(276, '2021-05-24 22:22:16', 0, 45, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(277, '2021-05-24 22:22:35', 0, 45, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(278, '2021-05-24 22:29:12', 0, 45, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(279, '2021-05-24 22:31:26', 0, 45, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(280, '2021-05-24 22:32:38', 0, 45, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(281, '2021-05-24 22:32:54', 0, 52, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(282, '2021-05-24 22:33:25', 0, 45, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(283, '2021-05-24 22:40:06', 0, 52, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(284, '2021-05-24 22:41:10', 0, 45, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan'),
(285, '2021-05-24 22:41:39', 0, 45, 'Gerung', '', 'RECALL_SPV_FROM_AGENT_NEW', 'Tofan'),
(286, '2021-05-24 22:51:01', 0, 45, 'Gerung', '', 'NEW_RE_ASSIGNMENT_SPV_TO_AGENT', 'Tofan');
...
--
-- Dumping data for table `event_audit_trail`
--

INSERT INTO `event_audit_trail` (`ID`, `EVENT_TIME`, `AGENT_USERNAME`, `EVENT_TABLE`, `EVENT_STATUS`, `EVENT_OPERATION`, `EVENT_DESCRIPTION`, `FAILURE_AMOUNT`, `BLOCKED_AMOUNT`) VALUES
(4, '2021-05-24 09:24:14', 'tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM Gerung TO SPV tofan TOTAL DATA : 1', '', ''),
(6, '2021-05-24 10:02:16', 'Intan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM tofan TO MANAGER Intan TOTAL DATA : 1', '', ''),
(7, '2021-05-24 10:48:16', 'Intan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM tofan TO MANAGER Intan TOTAL DATA : 1', '', ''),
(8, '2021-05-24 10:55:16', 'Intan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM gerung TO MANAGER Intan TOTAL DATA : 1', '', ''),
(9, '2021-05-24 11:21:53', 'busi', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM Intan TO ADMINISTRATOR busi TOTAL DATA : 1', '', ''),
(10, '2021-05-24 12:52:19', 'busi', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM tofan TO ADMINISTRATOR busi TOTAL DATA : 1', '', ''),
(11, '2021-05-24 13:06:11', 'busi', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM gerung TO ADMINISTRATOR busi TOTAL DATA : 1', '', ''),
(12, '2021-05-24 13:47:47', 'busi', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM Intan TO ADMINISTRATOR busi TOTAL DATA : 1', '', ''),
(13, '2021-05-24 13:50:55', 'Busi', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO MGR Intan TOTAL DATA : 1', '', ''),
(14, '2021-05-24 13:52:30', 'Busi', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO MGR Intan TOTAL DATA : 1', '', ''),
(15, '2021-05-24 13:58:30', 'busi', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM Intan TO ADMINISTRATOR busi TOTAL DATA : 1', '', ''),
(16, '2021-05-24 14:02:27', 'Busi', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO MGR Intan TOTAL DATA : 1', '', ''),
(17, '2021-05-24 14:02:52', 'Busi', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO SPV Budi TOTAL DATA : 0', '', ''),
(18, '2021-05-24 14:02:53', 'Busi', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO SPV Tofan TOTAL DATA : 1', '', ''),
(19, '2021-05-24 14:04:40', 'Busi', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO AGENT Gerung TOTAL DATA : 1', '', ''),
(20, '2021-05-24 14:06:36', 'Intan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO SPV Budi TOTAL DATA : 0', '', ''),
(21, '2021-05-24 14:06:37', 'Intan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO SPV Tofan TOTAL DATA : 1', '', ''),
(22, '2021-05-24 14:07:12', 'Intan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(23, '2021-05-24 14:08:14', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO AGENT Gerung TOTAL DATA : 2', '', ''),
(24, '2021-05-24 14:16:23', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  tes user4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(25, '2021-05-24 14:19:31', 'Intan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  tess66 TO SPV Budi TOTAL DATA : 0', '', ''),
(26, '2021-05-24 14:19:31', 'Intan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  tess66 TO SPV Tofan TOTAL DATA : 1', '', ''),
(27, '2021-05-24 21:33:43', 'Intan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  tess66 FROM Tofan TO MANAGER Intan TOTAL DATA : 1', '', ''),
(28, '2021-05-24 21:53:20', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  tes user4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(29, '2021-05-24 21:53:56', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(30, '2021-05-24 22:04:39', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(31, '2021-05-24 22:04:53', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(32, '2021-05-24 22:06:36', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(33, '2021-05-24 22:07:30', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  tes user4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(34, '2021-05-24 22:15:11', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test FROM Gerung TO SPV Tofan TOTAL DATA : 3', '', ''),
(35, '2021-05-24 22:15:49', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(36, '2021-05-24 22:18:52', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  tes user4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(37, '2021-05-24 22:21:47', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test TO AGENT Gerung TOTAL DATA : 2', '', ''),
(38, '2021-05-24 22:22:16', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(39, '2021-05-24 22:22:35', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(40, '2021-05-24 22:29:12', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(41, '2021-05-24 22:31:26', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(42, '2021-05-24 22:32:38', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(43, '2021-05-24 22:32:54', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  tes user4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(44, '2021-05-24 22:33:25', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(45, '2021-05-24 22:40:06', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  tes user4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(46, '2021-05-24 22:41:10', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test4 TO AGENT Gerung TOTAL DATA : 1', '', ''),
(47, '2021-05-24 22:41:39', 'Tofan', 'MASTER_CUSTOMER', 'RELOAD DATA', '127.0.0.1', 'RELOAD CAMPAIGN_NAME :  test4 FROM Gerung TO SPV Tofan TOTAL DATA : 1', '', ''),
(48, '2021-05-24 22:51:01', 'Tofan', 'MASTER_CUSTOMER', 'DISTRIBUTION DATA', '127.0.0.1', 'DISTRIBUTION CAMPAIGN_NAME :  test4 TO AGENT Gerung TOTAL DATA : 1', '', '');
...
(42, 0, '2021-05-15 12:29:13', '2021-07-14 12:29:13', '2021-05-24 22:21:47', NULL, NULL, 'Gerung', NULL, NULL, NULL, NULL, 'ALL', '1', '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '0', NULL, NULL, '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'test', 'Intan', 'Tofan', NULL, NULL, '5', 'nama5', NULL, NULL, NULL, '1245', NULL, NULL, '38', NULL, NULL, 'L', NULL, NULL, NULL, '08888888888', '', '', NULL, 'VISAMERAH', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '1', 'PARTNER', 'SAVING', 'REGULAR', 'SILVER', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(43, 0, '2021-05-15 12:29:13', '2021-07-14 12:29:13', '2021-05-24 22:21:47', NULL, NULL, 'Gerung', NULL, NULL, NULL, NULL, 'ALL', '1', '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '0', NULL, NULL, '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'test', 'Intan', 'Tofan', NULL, NULL, '6', 'nama6', NULL, NULL, NULL, '1246', NULL, NULL, '39', NULL, NULL, 'L', NULL, NULL, NULL, '08888888998', '', '', NULL, 'VISAMERAH', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '1', 'PARTNER', 'SAVING', 'REGULAR', 'SILVER', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(44, 0, '2021-05-15 12:29:14', '2021-07-14 12:29:14', '2021-05-24 14:04:40', NULL, NULL, '', NULL, NULL, NULL, NULL, 'ALL', '1', '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '0', NULL, NULL, '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'test', 'Intan', 'Tofan', NULL, NULL, '9', 'nama9', NULL, NULL, NULL, '1249', NULL, NULL, '42', NULL, NULL, 'P', NULL, NULL, NULL, '08889255555', '', '', NULL, 'VISAMERAH', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '2', 'PARTNER', 'SAVING', 'REGULAR', 'SILVER', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(45, 0, '2021-05-15 12:41:16', '2295-02-27 12:41:16', '2021-05-24 22:51:01', NULL, NULL, 'Gerung', NULL, NULL, NULL, NULL, 'ALL', '1', '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '0', NULL, NULL, '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'test4', 'Intan', 'Tofan', NULL, NULL, '5', 'nama5', NULL, NULL, NULL, '1245', NULL, NULL, '38', NULL, NULL, 'P', NULL, NULL, NULL, '08888876755', '', '', NULL, 'VISAMERAH', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '1', 'PARTNER', 'SAVING', 'REGULAR', 'SILVER', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
...
(47, 0, '2021-05-18 17:27:22', '2021-07-17 17:27:22', '2021-05-24 14:19:31', NULL, NULL, '', NULL, NULL, NULL, NULL, 'ALL', NULL, '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '0', NULL, NULL, '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'tess66', 'Intan', '', NULL, NULL, '5', 'nama99', NULL, NULL, NULL, '1245', NULL, NULL, '39', NULL, NULL, 'L', NULL, NULL, NULL, '08888879999', '', '', NULL, 'VISAMERAH', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '2', 'PARTNER', 'SAVING', 'REGULAR', 'SILVER', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
...
(52, 0, '2021-05-19 00:44:32', '2021-07-18 00:44:32', '2021-05-24 22:32:54', NULL, NULL, '', NULL, NULL, NULL, NULL, 'ALL', '1', '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '0', NULL, NULL, '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'tes user4', 'Intan', 'Tofan', NULL, NULL, '5', 'nama99', NULL, NULL, NULL, '1245', NULL, NULL, '39', NULL, NULL, 'L', NULL, NULL, NULL, '08888879999', '', '', NULL, 'VISAMERAH', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '2', 'PARTNER', 'SAVING', 'REGULAR', 'SILVER', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
...
(1, 'Teguh', 'Teguh Muhammad Harits', '$2y$10$Lna1X.lJGWo3q4iaD73TPuuS6PY65Hcb2LmvA/QpSXs13YtQkO5dK', 'ADMIN', '083135351881', 'ACTIVE', NULL, NULL, NULL, NULL, 'Intan', NULL, NULL, NULL, '2021-06-17', NULL, '2021-05-18', NULL, 'TEAM 1', 'ADMIN', 'NULL', NULL, 'NULL', 0, 0, 'NULL', 'NULL', 0, NULL, NULL, 'N', 'aries.jpg', NULL, NULL, NULL, NULL, NULL, NULL, NULL, '127.0.0.1', 'Animates 04', '2021-05-17 18:08:31', '2021-05-24 04:17:26'),
(2, 'Tofan', 'Tofan Maiki', '$2y$10$qGQWiys8iddd4iFMe4t0.OnDlFACabB/iavovD2VAR/siNFKj73r2', 'SPV', '0831391838191', 'ACTIVE', NULL, NULL, NULL, NULL, 'Intan', NULL, NULL, 'preview', '2021-06-17', NULL, '2021-05-17', NULL, 'TEAM 1', 'ADMIN', 'NULL', NULL, 'NULL', 1, 0, 'NULL', 'NULL', 1, NULL, NULL, 'N', 'shoes1.jpg', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'Animates 04', '2021-05-17 18:14:42', '2021-05-24 14:42:49'),
...
(4, 'Intan', 'Intan 08928983924', '$2y$10$xOePSucJHe8PzQxbrVc6ZOthLKr58krtBi7ZgNZ8Hbcz.ytOiDPUy', 'MANAGER', '0898898989', 'ACTIVE', NULL, NULL, NULL, NULL, 'Intan', NULL, NULL, 'preview', '2021-06-17', NULL, '2021-05-09', NULL, 'TEAM 1', 'ADMIN', 'NULL', NULL, 'NULL', 0, 0, 'NULL', 'NULL', 1, NULL, NULL, 'N', 'default.jpg', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'Animates 04', '2021-05-17 18:30:14', '2021-05-24 14:41:12'),
...
(1, 'Teguh', '$2y$10$Vi1RG0iG6sJBmOwo.eowtOI/vDHRhHdYtnFUGiGCzld/Rgs8yHZTG', 'Teguh Muhammad Harits', '127.0.0.1', '2021-05-24 11:17:26', 'admin', 0, 'aries.jpg', '2021-06-17', 'QZA0MyOOQIQgXl4skt1hjYDcqPwfs12UuMwMiYHjnL1GfebW97w2zG9UjYfS', '2021-05-18 01:08:31', '2021-05-24 04:17:26'),
(2, 'Tofan', '$2y$10$PG3a6egpfNQtb5IMQpKBhO8nuWUeVScL4F.NPgiDtE1CPBIcTsY6q', 'Tofan Maiki', '127.0.0.1', '2021-05-24 14:18:51', 'spv', 0, 'shoes1.jpg', '2021-06-17', '6AF7uqUWzowMYsYZibT5Z2MmXmSw2I84rCZZt5Gwki9Sxj8N65cpMT7AjqFo', '2021-05-18 01:14:42', '2021-05-24 14:42:49'),
...
(4, 'Intan', '$2y$10$tFhW3BkwERfTJQjRxVJn8uVGtVehoQTPZg.O8.KaeZrfI8P//Z2Mu', 'Intan 08928983924', '127.0.0.1', '2021-05-24 21:41:12', 'manager', 0, 'default.jpg', '2021-06-17', '2ZEIGVHeUwXwWUDjZC70MqmcBVrgzWffGD5rhI8jTD0v0CZzRFlmT6uoViBQ', '2021-05-18 01:30:14', '2021-05-24 14:41:12'),
